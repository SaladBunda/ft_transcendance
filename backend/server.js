...
let movementSpeed = 2;
let lastSpeedIncrease = Date.now();
let countdownInterval = null; // Track interval to clear it
...
function resetBall(loser = null) {
  ...
  gameState.ball = { x: CANVAS_WIDTH / 2, y: startY, dx: 0, dy: 0 };
  gameState.countdown = 3;
  gameState.gameActive = false; // Stop game during countdown
...
function resetGame() {
  ...
  gameState = {
    ...
    player1: { x: 20, y: (CANVAS_HEIGHT - PADDLE_HEIGHT) / 2, dy: 0, score: 0 },
    player2: { x: 570, y: (CANVAS_HEIGHT - PADDLE_HEIGHT) / 2, dy: 0, score: 0 },
    winner: null, // This is the key fix!
    countdown: 0,
    gameActive: false,
  };
...
fastify.register(async (fastify) => {
  fastify.get("/ws", { websocket: true }, (connection) => {
    ...
    connection.socket.on("message", (message) => {
      ...
      if (data.type === "update" && gameState.gameActive && !gameState.winner) {
